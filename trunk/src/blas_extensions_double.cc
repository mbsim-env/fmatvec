...
namespace fmatvec {
  ...
  void myblas_daxpy(const enum CBLAS_TRANSPOSE  transa, const enum CBLAS_TRANSPOSE transb, int m, int n, double alpha, const double *a, int lda1, double *b, int ldb1) {
    ...
    if(nota && notb && lda1==m && ldb1==m) {
      nn = (m)*(n);
      daxpy(nn, alpha, a, one, b, one);
    } else {
      if(nota) {
	if(notb) {
	  for(i = 0; i < n; i++)
	    daxpy(m, alpha, a+i*(lda1), one, b+i*(ldb1), one);
	} else {
	  for(i = 0; i < n; i++)
	    daxpy(m, alpha, a+i*(lda1), one, b+i, ldb1);
	}
      } else {
	if(notb) {
	  for(i = 0; i < n; i++)
	    daxpy(m, alpha, a+i, lda1, b+i*(ldb1), one);
	} else {
	  for(i = 0; i < n; i++)
	    daxpy(m, alpha, a+i, lda1, b+i, ldb1);
	}
      }
    }
  }
  ...
  void myblas_dcopy(const enum CBLAS_TRANSPOSE  transa, const enum CBLAS_TRANSPOSE transb, int m, int n, const double *a, int lda1, double *b, int ldb1) {
    ...
    if(nota && notb && lda1==m && ldb1==m) {
      nn = m*n;
      dcopy(nn,  a, one, b, one);
      return;
    }
    if(nota) {
      if(notb) {
	for(i = 0; i < n; i++)
	  dcopy(m,  a+i*lda1, one,  b+i*ldb1, one);
	return;
      } else {
	for(i = 0; i < n; i++)
	  dcopy(m,  a+i*lda1, one, b+i, ldb1);
	return;
      }
    } else {
      if(notb) {
	for(i = 0; i < n; i++)
	  dcopy(m,  a+i, lda1, b+i*ldb1, one);
	return;
      } else {
	for(i = 0; i < n; i++)
	  dcopy(m,  a+i, lda1, b+i, ldb1);
	return;
      }
    }
  }
  ...
  void myblas_dscal(const enum CBLAS_TRANSPOSE  transa, int m, int n, double alpha, const double *a, int lda1, double *c, int ldc) {
    ...
    if(nota && lda1==m && ldc==m) {
      nn = m*n;
      myblas_dscal(nn, alpha, a, one, c);
      return;
    }
    if(nota) {
      for(i = 0; i < n; i++)
	myblas_dscal(m, alpha, a+i*lda1, one, c+i*ldc);
    } else {
      for(i = 0; i < n; i++)
	myblas_dscal(m, alpha, a+i, lda1, c+i*ldc);
    }
  }
  ...
  void myblas_dscal(char transa, int m, int n, double alpha, double *a, int lda1) {
    ...
    if(nota && lda1==m) {
      nn = m*n;
      dscal(nn, alpha, a, one);
      return;
    }
    if(nota) {
      for(i = 0; i < n; i++)
	dscal(m, alpha, a+i*lda1, one);
    }
  }

  void myblas_daxpy(int n, double da, const double *dx, int incx, const double *dy, int incy, double *c)
  {
    int i, m;
    /* for(i=0; i<n; i++)
     *c++=*dx++ + *dy++;
  ...
  void myblas_daxpy(const enum CBLAS_TRANSPOSE transa, const enum CBLAS_TRANSPOSE transb, int m, int n, double alpha, const double *a, int lda1, const double *b, int ldb1, double *c, int ldc)
  {
    ...
    {
      nn = (m)*(n);
      myblas_daxpy(nn, alpha, a, one, b, one, c);
    }
    else
    {
      if(nota)
	...
	{
	  for(i = 0; i < n; i++)
	    myblas_daxpy(m, alpha, a+i*(lda1), one, b+i*(ldb1), one, c+i* ldc);
	}
	else
	{
	  for(i = 0; i < n; i++)
	    myblas_daxpy(m, alpha, a+i*(lda1), one, b+i, ldb1, c+i* ldc);
	}
      }
      else
      {
	...
	{
	  for(i = 0; i < n; i++)
	    myblas_daxpy(m, alpha, a+i, lda1, b+i*(ldb1), one, c+i* ldc);
	}
	else
	{
	  for(i = 0; i < n; i++)
	    myblas_daxpy(m, alpha, a+i, lda1, b+i, ldb1,c+i* ldc);
	}
      }
    }
  }
