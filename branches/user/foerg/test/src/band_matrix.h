...
namespace fmatvec {
   ...
   * complex<double> 
   * */
  template <class AT> class Matrix<GeneralBand<Ref>, AT> {
   ...
   /// @cond NO_SHOW
    ...
    protected:
      ...
      inline void deepCopy(const Matrix<GeneralBand<Ref>, AT> &A); 
	...
	if(ini == INIT)
       ...
       * */
      Matrix(const Matrix<GeneralBand<Ref>, AT> &A) : memory(A.memory), ele(A.ele) , n(A.n), kl(A.kl), ku(A.ku) {
      }
       ...
       * FMATVEC_NO_INITIALIZATION.
       * */
      Matrix<GeneralBand<Ref>, AT>& resize(int n_, int kl_, int ku_) {
	n=n_;
	kl=kl_;
	ku=ku_;
	...
	return *this;
       ...
       * \return A reference to the calling matrix.
       * */
      Matrix<GeneralBand<Ref>, AT>& resize(int n_, int kl_, int ku_, Initialization ini, const AT &a=0) {
	n=n_;
	kl=kl_;
	ku=ku_;
	memory.resize(n*(kl+ku+1));
	...
	return *this;
       ...
       * */
      Matrix<GeneralBand<Ref>, AT>& operator=(const Matrix<GeneralBand<Ref>, AT> &A) {
	...
	return operator>>(A);
       ...
       * \return A reference to the calling matrix.
       * */
      inline Matrix<GeneralBand<Ref>, AT>& operator<<(const Matrix<GeneralBand<Ref>, AT> &A);
	...
	return ((i-j>kl) || (i-j<-ku)) ? zero : ele[ku+i+j*(kl+ku)];
       ...
       * See operator()(int) 
       * */
      inline const Vector<General<Ref>, AT> operator()(int i) const;
       ...
       * respectively.
       * */
      inline Vector<General<Ref>, AT> operator()(int i);
       ...
       * \return A reference to the calling matrix.
       * */
      Matrix<GeneralBand<Ref>, AT>& resize() {n=0;return *this;};
       ...
       * */
      inline Matrix<GeneralBand<Ref>, AT> copy() const;
       ...
       * \return A reference to the calling matrix.
       * */
      inline Matrix<GeneralBand<Ref>, AT>& init(const AT &a);
  ...
  template <class AT>
    inline Matrix<GeneralBand<Ref>, AT>& Matrix<GeneralBand<Ref>, AT>::operator>>(const Matrix<GeneralBand<Ref>, AT> &A) { 
  ...
  template <class AT>
    inline Matrix<GeneralBand<Ref>, AT>&  Matrix<GeneralBand<Ref>, AT>::init(const AT& val) {
  ...
  template <class AT>
    inline Vector<General<Ref>, AT> Matrix<GeneralBand<Ref>, AT>::operator()(int i) {
#ifndef FMATVEC_NO_BOUNDS_CHECK
      assert(i<=ku);
      assert(i>=-kl);
#endif

      //return Vector<General<Ref>, AT>(n-i,ku+kl+1,memory,ele[ku-i + (i>0?i:0)*(kl+ku+1)]);
      return Vector<General<Ref>, AT>(n-abs(i),ku+kl+1,true,memory,ele+ku-i + (i>0?i:0)*(kl+ku+1));
      //(ku-i,i>0?i:0));
    }

  template <class AT>
    inline const Vector<General<Ref>, AT> Matrix<GeneralBand<Ref>, AT>::operator()(int i) const {
#ifndef FMATVEC_NO_BOUNDS_CHECK
      assert(i<=ku);
      assert(i>=-kl);
#endif
  ...
  template <class AT>
    inline Matrix<GeneralBand<Ref>, AT> Matrix<GeneralBand<Ref>, AT>::copy() const {

      Matrix<GeneralBand<Ref>, AT> A(n,kl,ku);
      A.deepCopy(*this);
  ...
  template <class AT>
    inline Matrix<GeneralBand<Ref>, AT>::operator std::vector<std::vector<AT> >() {
  ...
  template <class AT>
    inline void Matrix<GeneralBand<Ref>, AT>::deepCopy(const Matrix<GeneralBand<Ref>, AT> &A) { 
